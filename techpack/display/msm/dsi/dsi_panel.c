...
#define DSI_PANEL_HUAXING_83112 6
...
#ifdef OEM_TARGET_PRODUCT_BILLIE
	TP_Panel = panel;
	DSI_ERR("%s: [%s]--------TP_Panel = panel----\n", __func__, TP_Panel->name);
	update_tpfw_notifier_call_chain(DRM_PANEL_UPDATE_TPFW_BLANK,NULL);
#endif
	if (rc) {
...
#ifdef OEM_TARGET_PRODUCT_EBBA
static bool first_backlight = false;
#endif
...
#ifdef OEM_TARGET_PRODUCT_EBBA
	if ((saved_backlight == 0) && (bl_lvl != 0))
		first_backlight = true;
#endif
...
#ifdef OEM_TARGET_PRODUCT_EBBA
	if (first_backlight) {
		count = mode->priv_info->cmd_sets[DSI_CMD_POST_ON_BACKLIGHT].count;
		if (!count) {
			DSI_ERR("This panel does not support DSI_CMD_POST_ON_BACKLIGHT mode.\n");
			goto error;
		}
		else {
			usleep_range(8000, 10000);
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_POST_ON_BACKLIGHT);
			DSI_ERR("Send DSI_CMD_POST_ON_BACKLIGHT cmds.\n");
		}
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-167mhz-osc-dsi-on-command",
	"qcom,mdss-169mhz-osc-dsi-on-command",
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-167mhz-osc-dsi-on-command-state",
	"qcom,mdss-169mhz-osc-dsi-on-command-state",
				...
				int topology_override)
{
	...
	}else if (strcmp(panel->name, "hx83112_fhd_video_tianma") == 0) {
		dsi_panel_name = DSI_PANEL_HUAXING_83112;
		DSI_ERR("Dsi panel name is DSI_PANEL_HUAXING_83112");
	}
		       ...
		       struct mipi_dsi_host *host)
{
	...
	mipi_proc_entry = proc_create(MIPI_PROC_NAME,0666 , NULL, &mipi_reg_proc_fops);
	if (!mipi_proc_entry)
...
int dsi_panel_set_cabc_mode(struct dsi_panel *panel, u32 cabc_mode)
{
	...
	dsi = &panel->mipi_device;
	rc = mipi_dsi_dcs_set_display_cabc(dsi, cabc_mode);
	if (rc < 0) {
		pr_err("Failed to update dcs cabc:%d\n", cabc_mode);
		return -EINVAL;
	}
	panel->cabc_mode = cabc_mode;
	return rc;
}
