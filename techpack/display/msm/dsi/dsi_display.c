...
exit:
	...
	if (rc <=0) {
		...
		panel->esd_gesture_recovery = true;
#endif
	}
release_panel_lock:
...
int dsi_display_get_lcd_register(struct drm_connector *connector, u8 *buf)
{
	...
	for(i = 0xDA; i < 0xDD; i++){
		rc = mipi_dsi_dcs_read_test(&panel->mipi_device, i,&ctrl, sizeof(ctrl));
		if (rc)
			pr_err("failed to write control display: %d\n", rc);
		buf[index] = ctrl;
		index++;
		pr_info("read 54H val = 0x%x,buf = 0x%x\n", ctrl,buf[i]);
	}
	error:
		mutex_unlock(&panel->panel_lock);
		return rc;
...
#else
	dsi_display_register_read(dsi_display, 0xD8, buf, len);
#endif
...
int dsi_display_change_osc_mode(struct drm_connector *connector, int osc_mode)
{
	struct dsi_display *dsi_display = NULL;
	struct dsi_panel *panel = NULL;
	struct dsi_bridge *c_bridge;
	...
	switch (osc_mode){
		case 0:
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_167MHZ_OSC_ON);
			DSI_INFO("send osc_mode 167MHZ\n");
			break;
		case 1:
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_169MHZ_OSC_ON);
			DSI_INFO("send osc_mode 169MHZ\n");
			break;
		default:
			DSI_ERR("osc_mode is not support!!!\n");
			break;
	}
...
int dsi_display_set_cabc_mode(struct drm_connector *connector, u32 cabc_mode)
{
	struct dsi_display *dsi_display = NULL;
	struct dsi_panel *panel = NULL;
	struct dsi_bridge *c_bridge;
...
error:
 ...
 int dsi_display_get_cabc_mode(struct drm_connector *connector, unsigned int* cabc_mode)
 {
	struct dsi_display *dsi_display = NULL;
	struct dsi_panel *panel = NULL;
	struct dsi_bridge *c_bridge;
