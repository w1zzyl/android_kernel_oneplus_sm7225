...
static ssize_t cabc_store(struct device *device,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	ssize_t ret;
	unsigned int cabc_temp;
	struct drm_connector *connector = to_drm_connector(device);

	ret = kstrtouint(buf, 0, &cabc_temp);
	if (ret)
		return 0;
	ret = dsi_display_set_cabc_mode(connector, cabc_temp);
	return count;
}
static ssize_t cabc_show(struct device *device,
	struct device_attribute *attr,
	char *buf)
{
	ssize_t ret;
	...
	ret = dsi_display_get_cabc_mode(connector,	&data);
	if (ret) {
		pr_err("kstrtoint failed. ret=%d\n", ret);
		return ret;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	dsi_display_get_lcd_register(connector, state);
	if (dsi_panel_name == DSI_PANEL_SAMSUNG_S6E3HC2) {
	...
	if (ddic_check_info == 1)
		...
		ret = scnprintf(buf, PAGE_SIZE, "%04d/%02d/%02d\n%02d:%02d:%02d:%03d.%01d\n%s\nID: %02X %02X %02X\n",
				panel_year, panel_mon, panel_day, panel_hour, panel_min, panel_sec, panel_msec_int,
					panel_msec_rem, stage_string_info, panel_code_info, panel_stage_info,
						panel_production_info);
	}  else if (dsi_panel_name == DSI_PANEL_HUAXING_83112) {
		stage_string_info = "STAGE:hx83112 fhd tianma video";
		ret = scnprintf(buf, PAGE_SIZE,
		"%04d/%02d/%02d\n%02d:%02d:%02d:%03d.%01d\n%s\n ID: %02X %02X %02X\n"
		"IC_V: %02d\n DDIC_Check_Result: %s\n Tool: %s\n",
					panel_year, panel_mon, panel_day, panel_hour, panel_min,
						panel_sec, panel_msec_int, panel_msec_rem,
							stage_string_info, state[0],
								state[1], state[2],
...
int osc_mode;
static ssize_t osc_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", osc_mode);
}

static ssize_t osc_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	struct drm_connector *connector = to_drm_connector(dev);

	ret = kstrtoint(buf, 10, &osc_mode);
	if (ret) {
		pr_err("%s kstrtoint failed. ret=%d\n", __func__, ret);
		return ret;
	}

	pr_err("node osc_mode=%d\n", osc_mode);

	ret = dsi_display_change_osc_mode(connector, osc_mode);
	if (ret)
		pr_err("change osc mode failed ret:%d\n", ret);
...
static DEVICE_ATTR_RW(osc_mode);
