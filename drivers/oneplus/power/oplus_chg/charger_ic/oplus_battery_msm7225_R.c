...
#ifdef OEM_TARGET_PRODUCT_EBBA
extern enum alarmtimer_restart recover_charge_hrtimer_func(
...
/* add weak function, don't allow pass default */
bool __weak oplus_gauge_ic_chip_is_null(void) {return true;};
bool __weak oplus_charger_ic_chip_is_null(void) {return true;};
struct oplus_chg_operations * __weak oplus_get_chg_ops(void) {return NULL;};
...
#ifdef VENDOR_EDIT
static void oplus_delay_reset_dummy_fastchg_work(struct work_struct *work)
{
	//struct smb_charger *chg = container_of(work, struct smb_charger,
	//		delay_reset_dummy_fastchg_work.work);
	...
	if (vbus_rising) {
		...
		if (oplus_warp_get_fastchg_started() == false && g_oplus_chip) {
			...
			if (oplus_warp_get_fastchg_dummy_started() == false) {
				schedule_delayed_work(&chg->delay_reset_dummy_fastchg_work,
					round_jiffies_relative(msecs_to_jiffies(400)));
			} else {
				oplus_warp_reset_fastchg_after_usbout();
				oplus_chg_wake_update_work();
			}
...
irqreturn_t usb_source_change_irq_handler(int irq, void *data)
{
...
#endif
#ifdef VENDOR_EDIT
	/* lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add to swarp otg */
	struct oplus_chg_chip *chip = g_oplus_chip;
	...
	if ((stat & APSD_DTC_STATUS_DONE_BIT) && !chg->uusb_apsd_rerun_done) {
		...
		if ((reg_value & (SDP_CHARGER_BIT)) || (reg_value & (CDP_CHARGER_BIT))) {
			if (first_run_flag && (reg_value & (CDP_CHARGER_BIT))) {
...
irqreturn_t typec_state_change_irq_handler(int irq, void *data)
{
	...
	/*lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for swarp otg*/
...
int smblib_init(struct smb_charger *chg)
{
	...
	/* lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for charging */
#ifdef OEM_TARGET_PRODUCT_EBBA
	INIT_DELAYED_WORK(&g_oplus_chip->usbtemp_recover_work, oplus_usbtemp_recover_work);
#else
	...
	INIT_DELAYED_WORK(&chg->delay_reset_dummy_fastchg_work,
		oplus_delay_reset_dummy_fastchg_work);
#endif
...
#ifdef OEM_TARGET_PRODUCT_EBBA
		if (chip)
			cancel_delayed_work_sync(&g_oplus_chip->usbtemp_recover_work);
#endif
...
#ifdef OEM_TARGET_PRODUCT_EBBA
	chg->connecter_temp_1 = usbtemp / 1000;
#endif
...
#ifdef OEM_TARGET_PRODUCT_EBBA
	chg->connecter_temp_2 = usbtemp / 1000;
#endif
	usbtemp_volt = oplus_turn_usbtemp_to_volt(usbtemp, origin_table, len);
	usbtemp_volt = usbtemp_volt / 1000;
...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	sim1_gpio = of_get_named_gpio_flags(node, "sim1-gpios", 0, NULL);
	if (gpio_is_valid(sim1_gpio))
		pr_err("%s: get sim1 gpio failed!!!\n", __func__);
...
#ifdef OEM_TARGET_PRODUCT_EBBA
	rc = smblib_get_iio_channel(chg, "skin_therm", &chg->iio.skin_therm_chan);
	if (rc < 0)
		return rc;
#endif
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (g_oplus_chip && (g_oplus_chip->vbatt_num == 2 || is_warp_support_single_batt_swarp() == true)) {
...
#endif
#ifdef OEM_TARGET_PRODUCT_EBBA
		if (qpnp_is_power_off_charging() == true && chg->disconnect_vbus == 1) {
			pr_info("usb port protect triggered in poweroff mode, online true!");
			val->intval = 1;
		}
#endif
		break;
...
#ifdef OEM_TARGET_PRODUCT_EBBA
		if (qpnp_is_power_off_charging() == true && chg->disconnect_vbus == 1) {
			pr_info("usb port protect triggered in poweroff mode, voltage 5V!");
			val->intval = 5000000;
		}
#endif
		break;
...
#ifdef OEM_TARGET_PRODUCT_EBBA
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_PD &&
			 !g_oplus_chip->chg_ops->oplus_usbtemp_monitor_condition())
...
//zhouhaikang@BSP.CHG.Basic,2020/11/2,charge_done to keep msoc FULL_SOC(100%) while battery FULL&IBAT=0. */
	...
	case POWER_SUPPLY_PROP_CONNECT_DISABLE:
		...
		if (val->intval)
			pr_err("get chg->disconnect_vbus = %d\n", val->intval);
		break;
	case POWER_SUPPLY_PROP_FASTCHG_STATUS:
		chg_type = oplus_warp_get_warp_ver();
		if (chg_type != 0)
			val->intval = 1;
		else val->intval = 0;
		break;
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	rc = smblib_masked_write(chg, USBIN_AICL_OPTIONS_CFG_REG,
			mask, USBIN_AICL_PERIODIC_RERUN_EN_BIT);
...
extern void oplus_stop_usb_peripheral(void);
void oplus_enable_usb_peripheral(bool enable)
{
	if(enable) {
		oplus_start_usb_peripheral();
	} else {
		oplus_stop_usb_peripheral();
	}
}
...
int oplus_chg_set_pd_config(void)
{
	...
	if (chip->vbatt_num == 2
		|| is_warp_support_single_batt_swarp()
		|| is_platform_fuel_gauge()) {
...
struct oplus_chg_operations  smb5_chg_ops = {
	...
	.enable_usb_peripheral = oplus_enable_usb_peripheral,
};
#endif /* VENDOR_EDIT */
...
static int card_tray_status_show(struct seq_file *m, void *v)
{
	...
	gpio_value = gpio_get_value_cansleep(sim1_gpio);

	pr_err("%s: get sim1 gpio82 value is %d\n",
		__func__, gpio_value);
...
static int card_tray_detect_create_proc(void)
{
	...
	trey_status_entry = proc_create("sim_tray_status", 0444, NULL, &card_tray_status_fops);
...
{
	remove_proc_entry("sim_tray_status", NULL);
}
...
#ifdef OEM_TARGET_PRODUCT_EBBA
	chg->video_call_on = &__g_video_call_on;
#endif
	chg->pd_disabled = 0;
...
#ifdef OEM_TARGET_PRODUCT_EBBA
	wakeup_source_init(oplus_chip->usbtemp_wakelock, "usbtemp_wakeup");
#endif
	chg->pre_current_ma = -1;
	g_oplus_chip = oplus_chip;
...
//Gang.Yan add for usbtemp
	...
	if (card_tray_detect_create_proc())
		pr_err("%s: failed creating card tray detect attr\n", __func__);
	...
	if (g_oplus_chip && g_oplus_chip->enable_shipmode){
		if (oplus_ship_check_is_gpio(g_oplus_chip) == true) {
			...
			if(g_oplus_chip->ship_mode) {
				...
				chg_err("not enter ship mode,power off after 15s\n");
			}
		}
#else
		}
