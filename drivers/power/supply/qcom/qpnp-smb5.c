...
#define USB_TEM1_ADC_CHANNEL_NAME       "gpio1_voltage"
#define USB_TEM2_ADC_CHANNEL_NAME       "gpio8_voltage"
...
#ifdef OEM_TARGET_PRODUCT_BILLIE
	OF_PROP_READ(node, "ibatmax-warm-lcd-on-ma",
			chg->ibatmax[BATT_TEMP_WARM_LCD_ON], retval, 1);
#endif
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		#ifdef CONFIG_DISABLE_TEMP_PROTECT
		if (val->intval < 4) {
			pr_debug("disable temp protect version, real capacity:%d\n", val->intval);
			val->intval = 3;
		}
		#endif
		break;
	...
	case POWER_SUPPLY_PROP_TEMP:
		...
		#ifdef CONFIG_DISABLE_TEMP_PROTECT
		pr_info("disable temp protect version, real temp:%d\n", val->intval);
		val->intval = 250;
		#endif
...
#ifdef OEM_TARGET_PRODUCT_BILLIE
	rc = smblib_write(chg, DEBUG_ACCESS_SNK_CFG_REG, 0x17);
#else
